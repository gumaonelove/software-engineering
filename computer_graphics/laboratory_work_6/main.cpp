...
//  lab_6_num_1
//
//  Created by Булат Гумеров on 04.04.2023.
//

#include <gl\glut.h>
#include <math.h>
#include <C:\DOMAHA\KG\textures\glaux.h>
#include <stdio.h>
#pragma comment(lib, "C:\\DOMAHA\\KG\\textures\\GLAUX.lib")
...
void Pererisovka(void)
{
	...
	// В зависимости от индекса режима визуализации выполняются необходимые операции с изображением
		...
		case 1: /*Загрузка изображения из файла *.bmp*/
			pImage = auxDIBImageLoadA("C:/dora3.bmp");
		...
		case 3:
			...
			break;
		case 4:
			glPixelTransferf(GL_RED_SCALE, 1.0f); //устанавливает режимы передачи пикселей.
...
int main(int argc, char* argv[])
{
	...
	glutCreateMenu(ObrabotkaMenu); glutAddMenuEntry("Очистить экран", 0);
	glutAddMenuEntry("Загрузить изображение .bmp", 1);
	glutAddMenuEntry("Сохранить текущее изображение", 2);
	glutAddMenuEntry("Отразить изображение относительно оси Х", 3);
	glutAddMenuEntry("Отображение только R", 4);
	glutAddMenuEntry("Инверсия R", 5);
